...
<head>
    <meta charset="UTF-8">
    <title>گزارش پرداخت‌ها - چاپ</title>
...
<body>
    ...
    <h3>گزارش پرداخت‌ها - {{ $building->name ?? 'نامشخص' }}</h3>
        ...
        <tbody>
            @foreach($payments as $payment)
                <tr>
                    ...
                    <td>{{ $payment->method == 'online' ? 'آنلاین' : 'نقدی' }}</td>
                    <td>{{ $payment->invoice->type == 'fixed' ? 'ثابت' : 'جاری' }}</td>
                    <td>{{ $payment->status == 'paid' ? 'پرداخت شده' : 'نامشخص' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
