            ...
            <tbody>
                @forelse($units as $unit)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('units.edit', [$building->id, $unit->id]) }}" class="btn btn-sm btn-warning">ویرایش</a>

                            <form action="{{ route('units.destroy', [$building->id, $unit->id]) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button class="btn btn-sm btn-danger" onclick="return confirm('آیا مطمئنید؟')">حذف</button>
                            </form>
                        </td>
                    </tr>
                @empty
