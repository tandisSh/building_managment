            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('building_name') is-invalid @enderror"
                           id="building_name" name="building_name" value="{{ old('building_name', $building->building_name) }}">
                    @error('building_name')
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('shared_utilities') is-invalid @enderror"
                            id="shared_utilities" name="shared_utilities">
                        <option value="0" {{ old('shared_utilities', $building->shared_utilities) == 0 ? 'selected' : '' }}>خیر</option>
                        <option value="1" {{ old('shared_utilities', $building->shared_utilities) == 1 ? 'selected' : '' }}>بله</option>
                    </select>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('address') is-invalid @enderror"
                              id="address" name="address" rows="3">{{ old('address', $building->address) }}</textarea>
                    @error('address')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-6">
                    ...
                    @if($building->document_path)
                        <div class="mt-2">
                            <a href="{{ asset('storage/' . $building->document_path) }}" target="_blank" class="btn btn-sm btn-outline-primary">
